...
def pick_new_location(exclude_photo=None):
    """Pick a random location, avoiding the same photo back-to-back."""
    if exclude_photo and len(LOCATIONS) > 1:
        pool = [loc for loc in LOCATIONS if loc["photo"] != exclude_photo]
    else:
        pool = LOCATIONS
...
def get_or_create_current_round():
    ...
    if current is None:
        last = Round.query.filter_by(finished=True).order_by(Round.id.desc()).first()
        prev_photo = json.loads(last.location).get("photo") if last else None
        loc = pick_new_location(prev_photo)
...
def finish_round(round_obj):
    ...
    if len(survivors) > 1:
        prev_photo = json.loads(round_obj.location).get("photo")
        loc = pick_new_location(prev_photo)
        new_round = Round(location=json.dumps(loc), finished=False)
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
        # Migrate: add columns that were added after initial db creation
        with db.engine.connect() as _conn:
            _cols = {row[1] for row in _conn.execute(db.text("PRAGMA table_info(user)")).fetchall()}
            if 'best_streak' not in _cols:
                _conn.execute(db.text("ALTER TABLE user ADD COLUMN best_streak INTEGER DEFAULT 0"))
            if 'total_wins' not in _cols:
                _conn.execute(db.text("ALTER TABLE user ADD COLUMN total_wins INTEGER DEFAULT 0"))
            _conn.commit()
